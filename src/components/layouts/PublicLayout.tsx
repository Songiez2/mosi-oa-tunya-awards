...
export default function PublicLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      {/* Navbar */}
      <header className="sticky top-0 left-0 right-0 z-50 glass-dark shadow-md border-b border-border/30">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Button 
              size="sm" 
              className="bg-gradient-gold text-primary-foreground font-bold hover:scale-105 transition-transform" 
            ...
            {user ? (
              ...
              <div className="hidden sm:flex items-center gap-2">
                <Button variant="ghost" size="sm" className="h-8 text-xs" onClick={() => navigate('/login')}>Login</Button>
                <Button size="sm" variant="secondary" className="h-8 text-xs font-semibold" onClick={() => navigate('/register')}>Register</Button>
              </div>
            )}
      ...
      {/* Main content wrapper with bottom padding to avoid overlapping the bottom nav */}
      <main className="min-w-0 pb-16">{children}</main>

      {/* Footer */}
      <footer className="bg-muted border-t border-border mt-16 mb-16">
